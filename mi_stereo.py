...
print("Computing mutual information between features and labels...")
for label_idx in range(num_labels):
    label_name = lb.classes_[label_idx]
    label_values = binary_labels[:, label_idx]
    mutual_infos[label_name] = []
    for feature_idx in range(num_features):
        feature_values = feature_activations[:, feature_idx]
        mi = mutual_info_score(label_values, feature_values)
        mutual_infos[label_name].append({
            'feature_index': feature_idx,
            'mutual_information': mi
        })

# Find features with highest mutual information for each label
top_features = {}
for label_name in mutual_infos:
    sorted_features = sorted(mutual_infos[label_name], key=lambda x: x['mutual_information'], reverse=True)
...
print(f"Mutual information results saved to '{output_file}'.")
